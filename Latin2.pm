...
######################################################################
#
# Latin2 - Source code filter to Latin-2 script
#
...
# functions of package Elatin2
    ...
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin2::fc';           }
    ...
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin2::fc_';               }
...
# functions of package Elatin2
        ...
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin2::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Elatin2::fc_';               $slash = 'm//'; }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin2::uc($char[$i]) ne Elatin2::fc($char[$i]))) {
...
=head1 Calling 'Elatin2::ignorecase()' (Latin2.pm provides)

Latin2.pm applies calling 'Elatin2::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ Of \N, \p, \P and \X (Latin2.pm provides)

Latin2.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
Latin2.pm removes 'bytes::' at head of function name.
